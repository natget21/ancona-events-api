...
export const getNotificationByUserId = async (req, res) => {
  const response = await db.get(collectionName, {userId: req.params.userId});
  res.json(response);
};
...
export const markAllAsRead = async (req, res) => {
  ...
  const notifications = await db.get(collectionName, { userId: userId, seen: false });
  console.log("notif ",notifications);
  const updatePromises = notifications.map(notification => {
    console.log("not ",notification._id);
    return db.update(collectionName, notification._id, { seen: true });
  });
...
export const markAsRead = async (req, res) => {
  ...
  const notification = await db.update(collectionName, notificationId, { seen: true });
